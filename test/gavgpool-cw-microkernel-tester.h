...
void Test(xnn_f16_gavgpool_cw_ukernel_function gavgpool, xnn_init_f16_gavgpool_params_fn init_params) const {
    std::random_device random_device;
    auto rng = std::mt19937(random_device());
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      for (size_t i = 0; i < channels(); i++) {
        ...
        ASSERT_GE(fp16_ieee_to_fp32_value(y[i]), y_min)
          << "at position " << i << ", elements = " << elements() << ", channels = " << channels();
        ASSERT_NEAR(fp16_ieee_to_fp32_value(y[i]), y_ref[i], std::max(1.0e-0f, 1.0e-1f * std::abs(y_ref[i])))
          << "at position " << i << ", elements = " << elements() << ", channels = " << channels();
      }
    }
  }
