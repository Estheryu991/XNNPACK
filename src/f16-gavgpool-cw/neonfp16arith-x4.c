    ...
    const union xnn_f16_gavgpool_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  const float16x4_t vmultiplier = vld1_dup_f16(&params->neonfp16arith.multiplier);
  const float16x4_t voutput_min = vld1_dup_f16(&params->neonfp16arith.output_min);
  const float16x4_t voutput_max = vld1_dup_f16(&params->neonfp16arith.output_max);
  ...
  while (channels >= 4) {
    ...
    vst1_f16(output, vout);
    output = (__fp16*) ((uintptr_t) output + 4 * sizeof(__fp16));
  ...
  while (channels >= 1) {
    ...
    float16x4_t vsum = vpadd_f16(vsum0, vsum0);
    vsum = vpadd_f16(vsum0, vsum0);
    ...
    vst1_lane_f16(output, vout, 0);
    output = (__fp16*) ((uintptr_t) output + 1 * sizeof(__fp16));
    channels -= 1;
  }
}
